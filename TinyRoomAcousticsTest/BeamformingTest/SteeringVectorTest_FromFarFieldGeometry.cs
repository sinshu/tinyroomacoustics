...
namespace TinyRoomAcousticsTest
{
    ...
    public class SteeringVectorTest_FromFarFieldGeometry
    {
        ...
        public void CheckWithDelayFilter_Case2()
        {
            ...
            var delayFilters = new Complex[][]
            {
                Filtering.CreateFrequencyDomainDelayFilter(dftLength, 0 * Math.Sqrt(2) / 2),
                Filtering.CreateFrequencyDomainDelayFilter(dftLength, 3 * Math.Sqrt(2) / 2),
                Filtering.CreateFrequencyDomainDelayFilter(dftLength, 6 * Math.Sqrt(2) / 2)
            };
